...
Java_com_norman_rtmp_rtmpclient_RtmpClient_nativeOpen(JNIEnv* env, jobject thiz, jstring url_,
    ...
    RTMP *rtmp = (RTMP *) rtmpPointer;//nativeAlloc分配的内存地址指针
   // rtmp = RTMP_Alloc();
    if (rtmp == NULL) {
 ...
 */
JNIEXPORT jint JNICALL
Java_com_norman_rtmp_rtmpclient_RtmpClient_nativeRead(JNIEnv* env, jobject thiz, jbyteArray data_,
